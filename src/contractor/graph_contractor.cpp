...
struct ContractorNodeData {
  ...
  ContractorNodeData(std::size_t number_of_nodes,
      ...
      : is_core(std::move(uncontracted_nodes_)),
        contractable(std::move(contractable_)),
        priorities(number_of_nodes),
  ...
  void Renumber(const std::vector<NodeID> &old_to_new) {
    tbb::parallel_invoke(
        [&] {
          util::inplacePermutation(priorities.begin(), priorities.end(),
                                   old_to_new);
        },
        [&] {
        ...
        [&] {
          util::inplacePermutation(contractable.begin(), contractable.end(),
                                   old_to_new);
        },
        [&] {
...
struct ContractionStats {
  ...
  ContractionStats()
      : edges_deleted_count(0),
        edges_added_count(0),
        original_edges_deleted_count(0),
        original_edges_added_count(0) {}
};

struct RemainingNodeData {
  RemainingNodeData() = default;
  RemainingNodeData(NodeID id, bool is_independent)
      : id(id), is_independent(is_independent) {}
  NodeID id : 31;
  bool is_independent : 1;
};

struct ThreadDataContainer {
  explicit ThreadDataContainer(int number_of_nodes)
...
inline bool Bias(const util::XORFastHash<> &fast_hash, const NodeID a,
                 const NodeID b) {
  const unsigned short hasha = fast_hash(a);
  const unsigned short hashb = fast_hash(b);

  // The compiler optimizes that to conditional register flags but without
...
void ContractNode(ContractorThreadData *data, const ContractorGraph &graph,
                  const NodeID node, std::vector<EdgeWeight> &node_weights,
  ...
  for (auto in_edge : graph.GetAdjacentEdgeRange(node)) {
    ...
    for (auto out_edge : graph.GetAdjacentEdgeRange(node)) {
      ...
      if (target == source) {
        if (path_weight < node_weights[node]) {
          if (RUNSIMULATION) {
            // make sure to prune better, but keep inserting this loop if it
            // should still be the best CAREFUL: This only works due to the
            // independent node-setting. This guarantees that source is not
            // connected to another node that is contracted
            ...
            // This guarantees that source is not connected to another node that
            // is contracted
    ...
    if (RUNSIMULATION) {
      ...
      search(heap, graph, number_of_targets, SIMULATION_SEARCH_SPACE_SIZE,
             max_weight, node);
    } else {
      const int constexpr FULL_SEARCH_SPACE_SIZE = 2000;
      search(heap, graph, number_of_targets, FULL_SEARCH_SPACE_SIZE, max_weight,
             node);
    }
    for (auto out_edge : graph.GetAdjacentEdgeRange(node)) {
      const ContractorEdgeData &out_data = graph.GetEdgeData(out_edge);
      ...
      if (path_weight < weight) {
        if (RUNSIMULATION) {
          ...
          inserted_edges.emplace_back(
              source, target, path_weight, in_data.duration + out_data.duration,
              in_data.distance + out_data.distance,
              out_data.originalEdges + in_data.originalEdges, node,
              SHORTCUT_ARC, FORWARD_DIRECTION_ENABLED,
              REVERSE_DIRECTION_DISABLED);
          ...
          inserted_edges.emplace_back(
              target, source, path_weight, in_data.duration + out_data.duration,
              in_data.distance + out_data.distance,
              out_data.originalEdges + in_data.originalEdges, node,
              SHORTCUT_ARC, FORWARD_DIRECTION_DISABLED,
              REVERSE_DIRECTION_ENABLED);
  ...
  if (!RUNSIMULATION) {
    ...
    for (std::size_t i = inserted_edges_size; i < iend; ++i) {
      ...
      for (std::size_t other = i + 1; other < iend; ++other) {
        ...
        if (inserted_edges[other].data.weight !=
            inserted_edges[i].data.weight) {
          continue;
        }
        if (inserted_edges[other].data.shortcut !=
            inserted_edges[i].data.shortcut) {
          continue;
        }
...
ContractionStats SimulateNodeContraction(
    ContractorThreadData *data, const ContractorGraph &graph, const NodeID node,
...
void RenumberGraph(ContractorGraph &graph,
...
void RenumberData(std::vector<RemainingNodeData> &remaining_nodes,
                  ...
                  ContractorNodeData &node_data, ContractorGraph &graph) {
  std::vector<NodeID> current_to_new_node_id(graph.GetNumberOfNodes(),
                                             SPECIAL_NODEID);
  ...
  for (const auto current_id :
       util::irange<std::size_t>(0, graph.GetNumberOfNodes())) {
...
void DeleteIncomingEdges(ContractorThreadData *data, ContractorGraph &graph,
                         const NodeID node) {
...
bool UpdateNodeNeighbours(ContractorNodeData &node_data,
                          ...
                          const ContractorGraph &graph, const NodeID node) {
  std::vector<NodeID> &neighbours = data->neighbours;
  neighbours.clear();
  ...
  for (auto e : graph.GetAdjacentEdgeRange(node)) {
    ...
    neighbours.push_back(u);
    node_data.depths[u] =
        std::max(node_data.depths[node] + 1, node_data.depths[u]);
  }
  // eliminate duplicate entries ( forward + backward edges )
  std::sort(neighbours.begin(), neighbours.end());
  neighbours.resize(std::unique(neighbours.begin(), neighbours.end()) -
  ...
  for (const NodeID u : neighbours) {
    if (node_data.contractable[u]) {
      node_data.priorities[u] = EvaluateNodePriority(
          SimulateNodeContraction(data, graph, u, node_data.weights),
          node_data.depths[u]);
    }
  }
  return true;
...
bool IsNodeIndependent(const util::XORFastHash<> &hash,
                       ...
                       ContractorThreadData *const data, const NodeID node) {
  ...
  for (auto e : graph.GetAdjacentEdgeRange(node)) {
    ...
    // tie breaking
    if (std::abs(priority - target_priority) <
...
std::vector<bool> contractGraph(ContractorGraph &graph,
                                ...
                                double core_factor) {
  timer t_reset, t_score, t_contract, t_restruct, t_select, t_total, t_renumber;
  t_reset.reset(), t_score.reset(), t_contract.reset(), t_select.reset(),
      t_restruct.reset(), t_total.reset();
  t_renumber.reset();
  t_total.start();
  BOOST_ASSERT(node_weights_.size() == graph.GetNumberOfNodes());
  util::XORFastHash<> fast_hash;

  // for the preperation we can use a big grain size, which is much faster
  // (probably cache)
  ...
  ContractorNodeData node_data{
      graph.GetNumberOfNodes(), std::move(node_is_uncontracted_),
      std::move(node_is_contractable_), std::move(node_weights_)};
  ...
  auto number_of_core_nodes =
      std::max<std::size_t>(0, (1 - core_factor) * number_of_nodes);
  ...
  util::Log() << "preprocessing " << number_of_nodes_to_contract << " ("
              << (number_of_nodes_to_contract / (float)number_of_nodes * 100.)
              << "%) nodes...";
  ...
  while (remaining_nodes.size() > number_of_core_nodes) {
    ...
    tbb::parallel_for(
        tbb::blocked_range<NodeID>(0, remaining_nodes.size(),
                                   IndependentGrainSize),
        [&](const auto &range) {
          ContractorThreadData *data = thread_data_list.GetThreadData();
          ...
          for (auto i = range.begin(), end = range.end(); i != end; ++i) {
            ...
            remaining_nodes[i].is_independent =
                IsNodeIndependent(hash, node_data.priorities,
                                  new_to_old_node_id, graph, data, node);
          }
        });
    t_select.stop();
    ...
    tbb::parallel_for(tbb::blocked_range<NodeID>(begin_independent_nodes_idx,
                                                 end_independent_nodes_idx,
                                                 ContractGrainSize),
                      [&](const auto &range) {
                        ContractorThreadData *data =
                            thread_data_list.GetThreadData();
    ...
    for (auto position : util::irange<std::size_t>(begin_independent_nodes_idx,
                                                   end_independent_nodes_idx)) {
    ...
    tbb::parallel_for(
        tbb::blocked_range<NodeID>(begin_independent_nodes_idx,
                                   end_independent_nodes_idx, DeleteGrainSize),
        [&](const auto &range) {
          ContractorThreadData *data = thread_data_list.GetThreadData();
          for (auto position = range.begin(), end = range.end();
               position != end; ++position) {
    ...
    for (auto &data : thread_data_list.data) {
      for (const ContractorEdge &edge : data->inserted_edges) {
        ...
        if (current_edge_ID != SPECIAL_EDGEID) {
          ...
          if (current_data.shortcut &&
              edge.data.forward == current_data.forward &&
    ...
    number_of_contracted_nodes +=
        end_independent_nodes_idx - begin_independent_nodes_idx;
    remaining_nodes.resize(begin_independent_nodes_idx);

    p.PrintStatus(number_of_contracted_nodes);
    // std::cout << end_independent_nodes_idx - begin_independent_nodes_idx
    //           << "\t";
    t_reset.stop();
    // std::cout << t_reset.total_time() << "\t" << t_score.total_time() << "\t"
    //           << t_contract.total_time() << "\t" << t_restruct.total_time()
    //           << "\t" << t_select.total_time() << "\t"
    //           << t_renumber.total_time() << "\t" << t_total.total_time()
    //           << std::endl;
  }
  t_renumber.start();
  ...
  std::cout << t_reset.total_time() << "\t" << t_score.total_time() << "\t"
            << t_contract.total_time() << "\t" << t_restruct.total_time()
            << "\t" << t_select.total_time() << "\t" << t_renumber.total_time()
            << "\t" << t_total.total_time() << std::endl;
